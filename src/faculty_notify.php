...
<main role="main">
  ...
  <tbody>
    ...
    while($row=mysqli_fetch_array($run))
    {
        ...
        print_r($ms_real);
        foreach($ms_real as $facl)
        {
            $z=explode(',',$facl);
            print_r($z);
            
        $message="$row[2] requesting you to take his period on $z[1] for section $z[2]";
        $timestamp=$row[6];
        $v=$row[9];
        //$slno=$count++;
      ...
      echo"<td><button class='mui-btn mui-btn--small mui-btn--primary' style='width:105px'>Agree</button><br><button class='mui-btn mui-btn--small mui-btn--danger'>Not Agree</button></td>";
     echo"</tr>";
      }
        }
    }
